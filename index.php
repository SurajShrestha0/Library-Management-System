...
<body>
    ...
    <div class="register">
        ...
        <form action="index.php" method="post">
            ...
            <input type="password" Name="Password" placeholder="Password" required pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[a-zA-Z\d]{8,}$" onchange="validatePassword(this)">
    ...
    if (isset($_POST['signin'])) {
        ...
        // Check if the password matches the database and meets the criteria
        $password_matches = strcasecmp($row['Password'], $p) == 0;
        $y = $row['Type'];
        $password_meets_criteria = preg_match('/^(?=.*[a-z])(?=.*[A-Z]).{8,}$/', $p);
        if ($password_matches && $password_meets_criteria && !empty($u) && !empty($p)) {
            // Successful login
            $_SESSION['UserId'] = $u;
